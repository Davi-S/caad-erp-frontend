...
export function useCreateProduct() {
    ...
    return useMutation({
        mutationFn: async (input: ProductCreateRequest) => {
            ...
            if (res.error) throw new Error(extractErrorMessage(res.error, "Falha ao criar produto."))
            return res.data
        },
        onSuccess: () => {
...
export function useUpdateProduct() {
    ...
    return useMutation({
        mutationFn: async ({ productId, input }: { productId: string; input: ProductUpdateRequest }) => {
            const res = await api.PATCH("/products/{product_id}", {
                params: { path: { product_id: productId } },
                body: input
            })
            if (res.error) throw new Error(extractErrorMessage(res.error, "Falha ao atualizar produto."))
            return res.data
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['products'] })
